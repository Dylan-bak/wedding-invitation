...
mkdirSync('assets/temp', { recursive: true });
...
// 1) 문틀 배경 (문 포함 전체)
await sharp(SRC).resize({ width: 1400 }).jpeg(jpg).toFile('assets/temp/frame.jpg');

// 2) 좌/우 문짝 — 화면에서 차지하는 폭에 맞춰 축소
await sharp(SRC)
  .extract({ left: D.L, top: D.T, width: D.C - D.L, height: D.B - D.T })
  .resize({ width: 900 }).jpeg(jpg).toFile('assets/temp/door-l.jpg');
await sharp(SRC)
  .extract({ left: D.C, top: D.T, width: D.R - D.C, height: D.B - D.T })
  .resize({ width: 900 }).jpeg(jpg).toFile('assets/temp/door-r.jpg');
