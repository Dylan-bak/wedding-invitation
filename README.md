...
지금은 **캡처 이미지**(`assets/photo/map-naver.jpg`). 손댈 것 없고 비용·키도 없다. 손가락으로 확대·이동하는 진짜 지도로 바꾸고 싶을 때만 아래를 본다 — 권고 = **카카오맵**(카드 등록 없이 무료, 한글 지명 정확).

| 방식 | 비용 | 필요한 것 | 지명 | 판정 |
|---|---|---|---|---|
| 캡처 이미지 (지금) | 0원 | 없음 | 네이버 그대로 | 유지 |
| 카카오맵 JavaScript API | 0원 | 카카오 계정 · 앱 등록 · 도메인 등록 | 한글 정확 | 바꾼다면 이것 |
| 네이버 지도 (NAVER Cloud Platform) | 무료 제공량 안 | NCP 계정 + **결제수단(카드) 등록 필수** | 한글 정확 | 비권장 |
| OpenStreetMap + Leaflet | 0원 | 없음 | 한글 일부만 | 비권장 |

네이버는 무료 제공량 안에서 쓰더라도 가입 단계에서 카드를 등록해야 한다. 청첩장 방문량(하객 수백 명)은 어느 쪽이든 무료 범위.

카카오 키 받는 법 — 사내망에서 카카오 로그인이 막히면 집 PC 에서 한다.

1. https://developers.kakao.com 접속 → 카카오 계정으로 로그인
2. 상단 **내 애플리케이션** → **애플리케이션 추가하기** → 앱 이름(예: 모청) · 사업자명(예: 개인) 입력 → 저장
3. 만든 앱 클릭 → 왼쪽 **앱 설정 > 플랫폼** → **Web 플랫폼 등록** → 사이트 도메인에 두 줄 등록
