...
문이 화면에서 차지하는 크기(= 카메라가 얼마나 앞인지)는 `tools/build-door.mjs` 의 `H` 로 정해진다. 세로가 짧을수록 화면 높이에 맞추느라 문이 커진다. 문이 화면에서 얼마나 아래에 놓이는지는 `PAD`(문 위에 붙이는 유리천장·아이비 띠 높이) — 키울수록 위가 채워지고 아래 계단이 줄어든다. 아이폰(390×844) 기준 실측.

| 대문 사진 | 문짝(좌) 폭 | 문 높이 | 문 상단 위치 |
|---|---|---|---|
| door-2 원본 | 187px | 480px | 90px |
| door-4 (AI 로 문 위를 채운 것) | 148px | 354px | 276px |
| **door-5 (지금)** | **190px** | **488px** | **329px** |
...
`PAD` 를 키워 위를 채운 만큼 문 아래 계단이 줄어든다 — 화면 높이가 정해져 있어 위아래가 서로 자리를 뺏는다. `PAD 627` 에서 문 위 329px · 문 아래 27px.
