...
| `assets/photo/map-naver.jpg` | 오시는 길 지도 **폴백** — 카카오 SDK 가 안 뜰 때 보이는 캡처(참고안 네이버 지도 2배 해상도, 확대 막대 잘라냄) |
...
| `tools/build-version.mjs` | 배포본 번호를 `index.html` 과 `assets/version.txt` 에 심는다. push 전 반드시 실행 - §4 배포본 번호 갱신 |
| `assets/version.txt` | 서버에 올라간 배포본 번호. 페이지가 이 값을 읽어 자기 번호와 다르면 스스로 다시 받는다 |
...
p 0.015~0.50   좌 rotateY(-θ) / 우 rotateY(+θ),  θ 0 → 110°  (기본 = 바깥쪽으로 열림)
...
사진·아이콘·음악 파일을 교체했을 때만 (이유는 아래 주의 참고)

```bash
node tools/build-version.mjs --assets
```

하는 일 = 지금 시각으로 배포본 번호를 만들어 `index.html` 안(`window.BUILD`)과 `assets/version.txt` 두 곳에 같이 심는다. 그 뒤 commit·push.

왜 필요한가 = 하객이 이미 받은 주소는 바꿀 수 없다. 그리고 카카오톡 인앱 브라우저에서 push 한 뒤에도 옛 화면이 계속 보이는 일이 실제로 있었다(사용자 관찰). 왜 서버에 다시 묻지 않는지는 미확인. 그래서 기전을 따지지 않고 확실한 수단을 쓴다 = 페이지가 켜질 때 스스로 `assets/version.txt` 를 받아 자기 번호와 비교하고, 다르면 `?v=<새 번호>` 를 붙인 주소로 한 번만 다시 받는다. 주소가 달라지면 어떤 브라우저도 저장해 둔 화면을 쓸 수 없다.

이 대책은 이번 배포 이후부터 듣는다. 지금 하객 폰에 남아 있는 화면에는 이 검사 코드가 없어 스스로 갱신하지 못한다.
...
|---|---|---|
| 번호가 같음 | 그대로 | 0회 |
| 새 배포본 있음 | 한 번 깜빡이며 최신 내용으로 | 1회 |
| 이미 `?v=` 주소인데 또 번호가 다름 | 그대로 | 0회 (무한 새로고침 차단) |

주의

- 번호를 안 갱신하고 push = 하객 화면이 옛 내용으로 남는다. 그 외에 깨지는 것은 없다
- `--assets` 는 사진 주소 40곳에 번호를 붙인다 → 이미 다 받아둔 하객도 사진 4.3MB 를 다시 받는다. 자산 파일을 실제로 교체했을 때만 쓴다
- 공유 미리보기 카드 `assets/og.jpg` 에는 번호를 붙이지 않는다. 카카오톡이 카드를 주소 기준으로 따로 저장해 두므로 주소가 바뀌면 카드가 깨진다
- 무한 새로고침을 막는 줄 = `index.html` 머리의 `URLSearchParams(location.search).get('v') === v` 비교. 이 줄을 지우면 배포 중 두 파일이 잠깐 어긋나는 순간에 새로고침이 반복될 수 있다
- 배포본 번호는 배포마다 반드시 달라야 한다. 같은 번호를 두 번 쓰면 위 차단 줄이 오작동한다 (그래서 초 단위 시각을 쓴다)
...
위를 채운 만큼 문 아래 계단이 줄어든다 — 화면 높이가 정해져 있어 위아래가 서로 자리를 뺏는다. `GLASS_SRC 160`(유리천장 원본 190px 중 위 160px 만 씀, 아래는 잘라냄) → `GLASS 126`(45% 로 누름) + `IVY_H 56`(아이비 띠 위쪽 1/3 만) 에서 문 위 161px · 문 아래 195px.
...
문이 열리며 화면이 커지는 **줌 효과 = 제거됨.** iOS 떨림 후보로 A/B 시험하다가 효과 없어 코드째 삭제(전환용 dev 모드도 함께). `--zoom` CSS 변수·`.photo` 의 `scale()` 모두 없다.

#### 아이폰에서 문이 떨리던 이유

아이폰은 스크롤을 굴리는 곳과 페이지 스크립트가 도는 곳이 분리돼 있어 스크롤 위치가 띄엄띄엄 전달된다. 거기에 부드럽게 보이려고 넣어둔 **90밀리초 뒤따라가기 보정**이 얹혀, 값이 늦게 도착할 때마다 각도가 멈칫했다 따라붙기를 초당 수십 번 반복한 것이 떨림으로 보였다.

넣은 대책 (전부 현재 코드에 있음. 되돌리면 떨림 재발)

| 대책 | 무엇을 바꿨나 | 왜 |
|---|---|---|
| 터치에서 보정 끄기 | `coarse` 면 보간 계수 = 1 (보정 없음) | 보정은 데스크톱 휠 전용. 휠은 한 칸이 100px 씩 튀어 메울 값이 있지만 손가락 스크롤은 값이 연속이라 메울 것이 없고 지연만 남는다 |
| 스크롤 위치 읽는 시점 | 스크롤 이벤트 → 매 프레임(`tick` 첫 줄) | iOS 는 스크롤 이벤트를 매 프레임 주지 않는다. 이벤트에만 기대면 걸러진 프레임에서 각도가 멈춘다 |
| 연출값 전달 경로 | `#hero` 에 상속 변수 6개 → 각 요소에 `transform`·`opacity` 직접 | 상속 변수를 쓰면 첫 화면 전체가 매 프레임 다시 계산된다. 직접 쓰면 그 요소만 바뀐다 |
| 스크롤 안내 점 | `top` 애니메이션 → `transform` + 문 열리면 정지 | `top` 은 매 프레임 배치 계산을 유발한다. 안내가 사라진 뒤에도 계속 돌고 있었다 |
| 진행률 분모 | `innerHeight` → 미리 잰 `hero.offsetHeight - sticky.offsetHeight` | 스크롤 중 주소창이 접히며 `innerHeight` 가 프레임마다 변해 진행률이 앞뒤로 튄다 |
| 배치 계산 강제 유발 | 스크롤마다 `offsetHeight` 읽기 → 화면 크기 바뀔 때만(`measure()`) | 스크롤 중 읽으면 그 자리에서 배치 계산이 강제로 돈다. 두 값 다 `svh`·`lvh` 라 주소창 접힘과 무관 |
| `body` 가로 넘침 | `overflow-x:hidden` → `overflow-x:clip` | `hidden` 은 iOS Safari 가 body 를 스크롤 컨테이너로 취급해 `position:sticky` 가 불안정 |

상승 화면의 이동 거리(34svh)는 `.svh-probe` 라는 보이지 않는 자로 픽셀을 재 둔다. `svh` 는 주소창이 접혀도 안 변하므로 화면 크기가 바뀔 때만 다시 잰다.

멈추는 조건 = 값 변화가 없어도 12프레임 더 돌린 뒤. 이벤트가 안 온 프레임과 진짜 정지를 구분하기 위함.
...
현재 상태 = 실기기에서 떨림 사라진 것 확인. 아래는 재발했을 때의 진단 순서.

#### 떨림이 재발하면 이 순서로

1단계 — 각도가 부드럽게 변하는지 숫자로 본다. 로컬 서버를 띄운 다음
...
```bash
node tools/measure-scroll.mjs http://localhost:4321/
```

문 열림 구간(진행률 0.08~0.66)을 100px 씩 굴려 통과시키고, 매 프레임의 간격과 문짝 각도를 기록해 요약을 뱉는다. 각도는 스크립트가 문짝 인라인 `style` 에 직접 쓴 `rotateY()` 값을 읽는다.

정상 출력 (390×844, PC 크롬 실측)

| 항목 | 뜻 | 정상 범위 |
|---|---|---|
| `fps` | 초당 프레임 수 | 60 이상 |
| `p95Dt` | 프레임 간격 상위 5% (ms) | 17 이하 |
| `maxDt` | 최악 프레임 간격 (ms) | 35 이하 |
| `thetaMax` | 문이 도달한 최대 각도 (도) | 110 |
| `distinctThetas` | 서로 다른 각도가 몇 개 나왔나 | 50 이상 |
| `biggestThetaJumps` | 한 프레임에 뛴 각도 최대치 (도) | 6 이하 |

`distinctThetas` 가 한 자리수거나 `biggestThetaJumps` 가 10도를 넘으면 각도가 띄엄띄엄 뛰는 것이고, 그것이 눈에는 떨림으로 보인다.

이 측정기는 PC 크롬에서 돈다 = 데스크톱 경로(보정 켜짐)만 확인한다. 아이폰 떨림 자체는 이 PC 에서 재현 불가 — 측정기는 "코드가 구조적으로 멀쩡한가" 를 보는 용도.

2단계 — 위 대책 표의 7건이 코드에 살아있는지 확인한다. 하나라도 되돌아가 있으면 그것이 원인.

3단계 — 7건이 다 살아있는데도 실기기에서만 떨린다면 다음 수단 = CSS 스크롤 연동 애니메이션(`animation-timeline: scroll()`). 스크롤을 굴리는 곳에서 애니메이션까지 처리하므로 값이 늦게 도착하는 문제가 구조적으로 생기지 않는다. iOS 26.0 부터 문법 지원, 26.4 부터 스크롤과 같은 곳에서 처리. 단 아이폰의 약 34%(iOS 26 미만)가 이 기능을 못 쓰므로 지금의 JS 경로를 폴백으로 남겨야 한다.
...
| 오시는 길 | 주소 + 네이버 지도 캡처 + 네이버 지도·카카오 내비·티맵 버튼(앱 없으면 웹·스토어) + 접이식 버스·지하철·자가용 안내. 지하철 칸에 평지 도보·셔틀, 자가용 칸에 식장 주차 = 예식장 블로그 링크 `.waylink` |
| 안내사항 | 탭 2개 = 식전 쿠키(사진 + 신부대기실 마감) · 셔틀 안내(사진 + 문구 + 블로그 버튼) |
| 마음 전하는 곳 | 신랑 측 · 신부 측 접이식. 각각 본인 + 아버지 계좌, 복사 버튼 |
| 공유 | 공유하기(`navigator.share`, 없으면 링크 복사) · 링크 복사하기 |

### 예식장 블로그 링크

토브헤세드 공식 블로그(`blog.naver.com/tovhesed`) 글 3개를 참고안과 같은 자리에 건다. 주소 = 네이버 단축 링크라 블로그 글 번호가 바뀌어도 그대로 쓴다.

| 문구 | 주소 | 붙는 자리 |
|---|---|---|
| 평지 도보 안내 | `m.site.naver.com/20Xcn` | 오시는 길 > 지하철 이용시 |
| 셔틀 안내 | `m.site.naver.com/20XcK` | 오시는 길 > 지하철 이용시 · 안내사항 > 셔틀 안내 탭 |
| 식장 주차 안내 | `m.site.naver.com/20XcB` | 오시는 길 > 자가용 이용시 |

셔틀 탭 버튼 = `.btn-line`(두께 36px, 본문 버튼과 같은 모양) · 오시는 길 안쪽 3개 = `.waylink`(두께 33px 알약). `.btn-line` 은 `button` 과 `a` 를 같이 받으므로 `text-align:center; text-decoration:none` 이 들어 있다.

### 오시는 길 지도

지금은 **카카오 실제 지도**(손가락으로 확대·이동). `index.html` 의 카카오 SDK `script` 태그에 **JavaScript 키**가 들어 있고, 좌표(토브헤세드 `37.5186142, 127.0327347`)에 마커를 찍는다.

**SDK 가 안 뜨면 뒤의 캡처 이미지(`assets/photo/map-naver.jpg`)가 그대로 보인다** — `.map__kakao`(위)와 `.map__fallback`(아래)을 겹쳐 두었다. 위 칸이 비면(오프라인·아래 도메인 미등록으로 키 거절) 아래 캡처가 드러난다. 그래서 지도가 안 떠도 빈 칸이 생기지 않는다.

키가 동작하려면 **JavaScript SDK 도메인에 배포 주소가 등록돼 있어야 한다**(아래 4번). 등록 전에는 캡처 이미지만 보인다.

무료 제공량은 카카오맵 Web SDK 일 300,000건 — 청첩장 방문량(하객 수백 명 × 몇 번 = 수천 건)의 100배 이상. 카카오맵 무료는 개발자 계정에서 **첫 번째로 카카오맵을 켠 앱** 1개에만 적용된다.

#### 카카오 키 — 발급·도메인 등록

카카오가 부르는 **애플리케이션**은 스마트폰 앱이 아니라 **키를 발급받는 단위**(프로젝트 이름). 웹사이트도 애플리케이션을 하나 만들고 **JavaScript 키**를 받아 쓴다. 앱을 만들거나 심사받는 절차는 없다. (콘솔 UI 갱신으로 예전 `앱 설정 > 플랫폼` 이 `앱 설정 > 앱` 한 곳으로 합쳐졌다.)

사내망에서 카카오 로그인이 막히면 집 PC 에서 한다.

1. https://developers.kakao.com 접속 → 카카오 계정으로 로그인
2. 상단 **내 애플리케이션** → **애플리케이션 추가하기** → 앱 이름(예: 모청) · 사업자명(예: 개인) 입력 → 저장
3. 만든 것 클릭 → 왼쪽 **앱 설정 > 앱 > 플랫폼 키** → **JavaScript 키** 복사 → 전달
4. 같은 **플랫폼 키** 페이지의 **JavaScript 키** 항목 아래 **JavaScript SDK 도메인**(= 이 키를 쓸 웹 주소)에 두 줄 등록
   - `https://dylan-bak.github.io`
   - `http://localhost:4321` (로컬 확인용)
5. 왼쪽 **제품 설정 > 카카오맵 > 사용 설정** → **상태**를 **ON**

JavaScript 키는 브라우저에 그대로 노출된다(정상). 4번의 도메인 등록이 보호 장치라, 다른 사이트에서 그 키를 써도 지도가 뜨지 않는다. 카카오맵 무료 제공량은 개발자 계정에서 **첫 번째로 카카오맵을 켠 앱** 1개에만 적용된다.
