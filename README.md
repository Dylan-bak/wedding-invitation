...
#### 시트 양식을 바꿀 때는 시트 이름도 올린다

`SHEET_NAME` 뒤의 번호를 올린다 (`rsvp` → `rsvp_v2` → `rsvp_v3`).
같은 이름을 두면 열 순서가 달라진 응답이 옛 줄과 섞여 읽을 수 없게 된다. 이름을 올리면 옛 응답은 옛 시트에 그대로 남는다.
...
스프레드시트 `rsvp_v2` 시트 — 방문자ID · 구분 · 참석여부 · 식사인원 · 성함 · 전달말씀 · 최초접수 · 최종수정 · 수정횟수.
...
**이름을 받지 않으므로 누가 오는지는 알 수 없고, 측별 인원 합계만 얻는다.**
