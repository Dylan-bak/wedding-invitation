...
8. **확인** — 사이트에서 직접 한 번 보내보고 스프레드시트 `모청 참석 회신 v1` 시트에 줄이 생기는지 본다.
...
`SHEET_NAME` 뒤 버전을 올린다 (`모청 참석 회신 v1` → `v2`).
...
스프레드시트 `모청 참석 회신 v1` 시트 — 방문자ID · 구분 · 참석여부 · 식사인원 · 성함 · 전달말씀 · 최초접수 · 최종수정 · 수정횟수.
