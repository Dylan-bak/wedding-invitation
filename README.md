...
카카오가 부르는 **애플리케이션**은 스마트폰 앱이 아니라 **키를 발급받는 단위**(프로젝트 이름). 웹사이트도 애플리케이션을 하나 만들고, 그 안에서 플랫폼을 **Web** 으로 등록한다. 앱을 만들거나 심사받는 절차는 없다.
...
3. 만든 것 클릭 → 왼쪽 **앱 설정 > 플랫폼** → **Web 플랫폼 등록** → 사이트 도메인에 두 줄 등록
   - `https://dylan-bak.github.io`
   - `http://localhost:4321` (로컬 확인용)
4. 왼쪽 **앱 설정 > 앱 키** → **JavaScript 키** 복사 → 전달
5. 왼쪽 **제품 설정 > 카카오맵** → 활성화 ON

JavaScript 키는 브라우저에 그대로 노출된다(정상). 3번의 도메인 등록이 보호 장치라, 다른 사이트에서 그 키를 써도 지도가 뜨지 않는다.
