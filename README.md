...
| `tools/build-version.mjs` | 배포본 번호 심기. 절차·이유 = §9 「고친 뒤 다시 올릴 때」 |
| `assets/version.txt` | 서버에 올라간 배포본 번호. 페이지가 이 값을 읽어 자기 번호와 다르면 스스로 다시 받는다 (§9) |
...
**올릴 것은 `index.html` + `assets/version.txt` + `assets/hero` + `assets/photo` + `assets/qr.*` 뿐.**
...
1. 배포본 번호 갱신 — 빼먹으면 이미 링크를 받은 하객 화면이 옛 내용으로 남는다
   ...
   사진·아이콘·음악 파일을 교체했을 때만 (이유 = 아래 주의)
...
지금 시각으로 배포본 번호를 만들어 `index.html` 안(`window.BUILD`)과 `assets/version.txt` 두 곳에 같이 심는다.

하객이 이미 받은 주소는 바꿀 수 없다. 그리고 카카오톡 인앱 브라우저에서 push 한 뒤에도 옛 화면이 계속 보이는 일이 실제로 있었다(사용자 관찰). 왜 서버에 다시 묻지 않는지는 미확인. 그래서 기전을 따지지 않고 확실한 수단을 쓴다 = 페이지가 켜질 때 스스로 `assets/version.txt` 를 받아 자기 번호와 비교하고, 다르면 `?v=<새 번호>` 를 붙인 주소로 한 번만 다시 받는다. 주소가 달라지면 어떤 브라우저도 저장해 둔 화면을 쓸 수 없다.
...
- 배포본 번호는 배포마다 반드시 달라야 한다. 같은 번호를 두 번 쓰면 무한 새로고침 차단 장치가 오작동한다 (그래서 초 단위 시각을 쓴다)

### 9-1. GitHub Pages — 무료 · 가장 쉬움

저장소가 **public** 이어야 무료다. private 이면 GitHub Pro 필요.

1. 저장소에 push (매번 하는 순서는 위 「고친 뒤 다시 올릴 때」)
