...
지금은 **캡처 이미지**(`assets/photo/map-naver.jpg`). 손댈 것 없고 비용·키도 없다. 손가락으로 확대·이동하는 실제 지도로 바꿀 때만 아래를 본다 — 권고 = **카카오맵**(결제수단 등록 없이 시작 가능).

무료 제공량은 어느 쪽이든 청첩장 방문량(하객 수백 명 × 몇 번 = 수천 건)을 한참 넘는다. 갈림길은 요금이 아니라 **가입 문턱**.

| 방식 | 무료 제공량 | 조건 | 판정 |
|---|---|---|---|
| 캡처 이미지 (지금) | 무제한 | 없음 | 유지 |
| 카카오맵 Web JavaScript SDK | 일 300,000건 | 카카오 계정 · 개발자 계정에서 **첫 번째로 활성화한 앱** 1개에만 무료 | 바꾼다면 이것 |
| 네이버 Web Dynamic Map (Ncloud) | 월 6,000,000건 | Ncloud 계정 · **대표 계정** 1개에만 무료(개인은 전화번호 기준 1계정, Maps 를 처음 쓴 계정이 대표 계정) | 대안 |
| 구글 지도 iframe (`output=embed`) | 키 없음 | 없음 | 비권장 — 국내 상호·건물 표기가 두 곳보다 적다 |

두 곳 모두 "계정 1개만 무료" 라 대표/첫 앱이 아닌 계정으로 호출하면 과금된다. 새로 만든 계정으로 이 청첩장에만 쓰면 그 계정이 대표/첫 앱이 된다.
