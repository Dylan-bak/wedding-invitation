...
p 0.015~0.50   좌 rotateY(-θ) / 우 rotateY(+θ),  θ 0 → 110°  (기본 = 바깥쪽으로 열림)
               줌 켠 판(§4 A/B)만 동시에 화면 zoom 1 → 1.22 (문 안으로 걸어 들어가는 느낌)
...
줌은 A/B 시험 중. 기본 = 줌 없음(`maxZoom = 1`). 첫 화면 "스크롤 해주세요" 를 탭 간격 1.2초 안으로 7번 연속 탭하면 줌 켠 판(dev 모드)으로 바뀌어 `alert('dev 모드입니다')` 가 뜨고 왼쪽 위에 붉은 `DEV · 줌 켬` 표식이 계속 보인다. `localStorage` 의 `zoom-on` 에 기억(다시 7번 = 기본판). 판정이 나면 `maxZoom` 초기값을 확정값으로 고정하고 탭 전환·표식 코드를 지운다.

탭이 먹으려면 두 가지가 유지돼야 한다 — `.reveal` 의 `pointer-events:none`(이게 없으면 `.hint` 를 화면 전체로 덮어 가린다) · `.hint` 의 `touch-action:manipulation`(없으면 두 번째 탭에서 브라우저가 화면을 확대해 3번째부터 먹지 않는다).

iOS 에서 문이 내려올 때 떨리는 현상에 넣은 대책 4건 (줌 끄기는 별개. 실기기 판정 = HANDOFF P3)

| 대책 | 무엇을 바꿨나 | 왜 |
|---|---|---|
| 진행률 분모 | `innerHeight` → 미리 잰 `hero.offsetHeight - sticky.offsetHeight` | iOS 는 스크롤 중 주소창이 접히며 `innerHeight` 가 프레임마다 변해 진행률 `p` 가 앞뒤로 튄다 |
| 배치 계산 강제 유발 | 스크롤마다 `offsetHeight`·`getBoundingClientRect` 읽기 → 화면 크기 바뀔 때만(`measure()`) | 스크롤 중 이 값을 읽으면 그 자리에서 배치 계산이 강제로 돌아 프레임이 밀린다. 두 값 다 `svh`·`lvh` 라 주소창 접힘과 무관 |
...
같이 넣은 것 = 매 프레임 투명도가 바뀌는 `.hero__logo`·`.hint` 에 `will-change:opacity`. 그림자 붙은 큰 글자를 다시 그리지 않고 합성 단계에서 처리한다.
